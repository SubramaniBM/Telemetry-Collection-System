...
def run_client(server_ip: str, server_port: int, client_id: int,
    ...
    Args:
        ...
        duration:    How long to send (seconds), 0 = infinite
    """
    ...
    print(f"[Client {client_id}] Sending telemetry to {server_ip}:{server_port}")
    print(f"[Client {client_id}] Rate: {rate} pkts/sec | Duration: {duration}s")
    print(f"[Client {client_id}] Packet size: {PACKET_SIZE} bytes")
    print()
    ...
    try:
        while True:
            ...
            if seq_num % 1000 == 0:
                print(f"[Client {client_id}] Sent {seq_num} packets ({elapsed:.1f}s elapsed)")
        ...
        actual_rate = seq_num / total_time if total_time > 0 else 0
        
        print()
        print(f"{'=' * 50}")
        print(f"  Client {client_id} — Summary")
        print(f"{'=' * 50}")
        print(f"  Total packets sent : {seq_num}")
        print(f"  Duration           : {total_time:.2f}s")
        print(f"  Actual send rate   : {actual_rate:.1f} pkts/sec")
        print(f"  Data sent          : {seq_num * PACKET_SIZE:,} bytes")
        print(f"{'=' * 50}")
